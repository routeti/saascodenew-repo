...
type IndexGetContactQuery = { name: string; number: string };
...
export const getContact = async (
  ...
  const { companyId } = req.user;
  const contact = await GetContactService({ name, number, companyId });
...
export const remove = async (
  ...
  const io = getIO();
  io.emit(`company-${companyId}-contact`, { action: "delete", contactId });
